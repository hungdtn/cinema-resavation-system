...
public interface MovieRepository extends JpaRepository<Movie, Integer>, JpaSpecificationExecutor<Movie> {
    ...
    @Query("SELECT p FROM Movie p JOIN p.theLoaiList t WHERE LOWER(t.tenTheLoai) = LOWER(:theLoai) AND p.trangThai = :trangThai")
    List<Movie> findByTheLoaiAndTrangThaiIgnoreCase(@Param("theLoai") String theLoai, @Param("trangThai") String trangThai);

    // 2. Lọc theo thể loại
    @Query("SELECT p FROM Movie p JOIN p.theLoaiList tl WHERE LOWER(tl.tenTheLoai) = LOWER(:tenTheLoai)")
    List<Movie> findByTheLoaiIgnoreCase(@Param("tenTheLoai") String tenTheLoai);
    ...
    List<Movie> findAllFetchHinhAnh();

    @Modifying
    @Query(value = "INSERT INTO phim (ten_phim, mo_ta, thoi_luong, ngay_phat_hanh, trang_thai, dinh_dang, ngay_tao) " +
            "VALUES (:tenPhim, :moTa, :thoiLuong, :ngayPhatHanh, :trangThai, :dinhDang, :ngayTao)",
            nativeQuery = true)
    ...
    @Query(value = "UPDATE phim SET ten_phim = :tenPhim, mo_ta = :moTa, thoi_luong = :thoiLuong, " +
            "ngay_phat_hanh = :ngayPhatHanh, trang_thai = :trangThai, dinh_dang = :dinhDang, ngay_tao = :ngayTao WHERE id_phim = :idPhim",
            nativeQuery = true)
